...
-- Sem Box, e linha central no Model
...
-- Calcula posição central de um model (posição média dos BaseParts)
local function getModelCenter(model)
    ...
    if count > 0 then
        ...
        return model:IsA("BasePart") and model.Position or Vector3.zero
    end
end

-- Aplica offset baseado na posição escolhida
...
function ESP:Add(target: Instance, nameOverride)
    ...
    table.insert(self.Objects, {
        ...
        Highlight = ESP.Settings.ShowOutline and Instance.new("Highlight", target),
